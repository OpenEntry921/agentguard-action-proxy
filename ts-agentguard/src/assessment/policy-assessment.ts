...
interface PolicySourceAssessmentBasis {
...
interface PolicyAssessmentResult extends AssessmentResult {
  ...
  policySourceAssessmentBasis: PolicySourceAssessmentBasis;
  gapAnalysis: string[];
}
...
const policyResults: Record<PolicySampleFile, PolicyAssessmentResult> = {
  "오픈은행_AI_정책.pdf": {
    ...
    domainScores: [
      ...
      { domain: "MODEL_GOVERNANCE_HUMAN_OVERSIGHT", label: "AI 운영관리", score: 45, maxScore: 100 },
      { domain: "FINANCIAL_ACTIONS", label: "금전 실행 관리", score: 66, maxScore: 100 },
    ],
    priorityRisks: [
      ...
      { domain: "FINANCIAL_ACTIONS", label: "외부 LLM 사용", score: 66, description: "외부 LLM 사용 신청, 승인, 사용 범위 기록 체계가 필요합니다." },
    ],
    recommendedActions: [
  ...
  "오픈카드_AI_정책.pdf": {
    ...
    domainScores: [
      ...
      { domain: "MODEL_GOVERNANCE_HUMAN_OVERSIGHT", label: "AI 운영관리", score: 52, maxScore: 100 },
      { domain: "FINANCIAL_ACTIONS", label: "금전 실행 관리", score: 70, maxScore: 100 },
    ],
    priorityRisks: [
      ...
      { domain: "PRIVACY_DATA_PROTECTION", label: "개인정보 보호", score: 74, description: "개인정보 입력 제한은 있으나 위반 탐지 및 사후 감사 증적이 부족합니다." },
      { domain: "FINANCIAL_ACTIONS", label: "외부 LLM 사용", score: 70, description: "외부 AI 서비스 사용 범위와 승인 결과를 중앙 대장으로 관리해야 합니다." },
    ],
    recommendedActions: [
  ...
  "오픈증권_AI_정책.pdf": {
    ...
    domainScores: [
      ...
      { domain: "PRIVACY_DATA_PROTECTION", label: "개인정보 보호", score: 74, maxScore: 100 },
      { domain: "MODEL_GOVERNANCE_HUMAN_OVERSIGHT", label: "AI 운영관리", score: 68, maxScore: 100 },
      { domain: "FINANCIAL_ACTIONS", label: "금전 실행 관리", score: 72, maxScore: 100 },
    ],
    priorityRisks: [
      ...
      { domain: "FINANCIAL_ACTIONS", label: "외부 LLM 사용", score: 72, description: "외부 LLM 승인 절차를 위험등급별로 차등화해야 합니다." },
      { domain: "PRIVACY_DATA_PROTECTION", label: "개인정보 보호", score: 74, description: "고객정보 입력 제한 정책의 교육 이수 및 점검 증적을 강화해야 합니다." },
      { domain: "STRATEGIC_GOVERNANCE", label: "AI 사용 감사", score: 76, description: "분기별 사용 현황과 예외 승인 내역을 감사 패키지로 정례화해야 합니다." },
    ],
    recommendedActions: [
...
function renderPolicySourceDetails(result: PolicyAssessmentResult): string {
  return `<div class="source-detail-grid"><div><span>분석 조항</span><strong>${result.policySourceAssessmentBasis.analyzedClauses}</strong></div><div><span>추출 통제</span><strong>${result.policySourceAssessmentBasis.extractedControls}</strong></div></div><div class="standards"><span>적용 표준 (평가 기준)</span><ul>${result.policySourceAssessmentBasis.assessmentStandards.map((standard) => `<li>${escapeHtml(standard)}</li>`).join("")}</ul><p>업로드된 정책을 어떤 기준으로 평가했는지 보여주는 Mock 기준입니다.</p></div>`;
}
...
export function policyAssessmentReportHtml(result: PolicyAssessmentResult): string {
  return `<!doctype html><html lang="ko"><head><meta charset="utf-8" /><meta name="viewport" content="width=device-width, initial-scale=1" /><title>${escapeHtml(result.organizationName)} AI 정책 평가보고서</title><style>:root { color-scheme: light; --bg: #eef1f5; --paper: #ffffff; --ink: #172033; --muted: #5d687a; --line: #d7dde7; --navy: #12213a; --blue: #1f4e79; --gold: #a67c00; --red: #9f2f2f; } * { box-sizing: border-box; } button, input, textarea, select { font-family: inherit; } body { margin: 0; min-height: 100vh; font-family: Helvetica, "Helvetica Neue", Arial, "Apple SD Gothic Neo", "Noto Sans KR", sans-serif; background: var(--bg); color: var(--ink); } .toolbar { position: sticky; top: 0; z-index: 2; display: flex; justify-content: flex-end; gap: 10px; padding: 14px 22px; background: rgba(238,241,245,.96); border-bottom: 1px solid var(--line); } .toolbar a, .toolbar button { border: 1px solid var(--navy); border-radius: 4px; background: var(--navy); color: white; padding: 10px 14px; font: 700 .9rem Helvetica, "Helvetica Neue", Arial, "Apple SD Gothic Neo", "Noto Sans KR", sans-serif; text-decoration: none; cursor: pointer; } .toolbar a { background: white; color: var(--navy); } main { width: min(960px, calc(100% - 32px)); margin: 28px auto 42px; } .report { background: var(--paper); border: 1px solid var(--line); box-shadow: 0 18px 45px rgba(18,33,58,.13); padding: clamp(30px, 5vw, 58px); } header { display: grid; grid-template-columns: 1fr auto; gap: 24px; padding-bottom: 24px; border-bottom: 3px solid var(--navy); } .eyebrow { color: var(--blue); font: 600 .78rem Helvetica, "Helvetica Neue", Arial, "Apple SD Gothic Neo", "Noto Sans KR", sans-serif; letter-spacing: .16em; text-transform: uppercase; } h1 { margin: 10px 0 0; color: var(--navy); font-size: clamp(2rem, 4.4vw, 3.7rem); line-height: 1.02; letter-spacing: -.03em; } .meta { min-width: 280px; border: 1px solid var(--line); } .meta div { display: flex; justify-content: space-between; gap: 16px; padding: 11px 13px; border-bottom: 1px solid var(--line); font: 700 .88rem Helvetica, "Helvetica Neue", Arial, "Apple SD Gothic Neo", "Noto Sans KR", sans-serif; } .meta div:last-child { border-bottom: 0; } .meta span { color: var(--muted); } section { padding: 26px 0; border-bottom: 1px solid var(--line); break-inside: avoid; } h2 { margin: 0 0 18px; color: var(--navy); font: 600 1rem Helvetica, "Helvetica Neue", Arial, "Apple SD Gothic Neo", "Noto Sans KR", sans-serif; letter-spacing: .13em; text-transform: uppercase; } p, li { color: var(--muted); line-height: 1.68; } .score-grid, .roadmap { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; } .metric, .roadmap-card, .box { border-left: 4px solid var(--blue); background: #f7f9fc; padding: 14px; } .metric span { display: block; color: var(--muted); font: 600 .76rem Helvetica, "Helvetica Neue", Arial, "Apple SD Gothic Neo", "Noto Sans KR", sans-serif; text-transform: uppercase; } .metric strong { display: block; margin-top: 8px; color: var(--navy); font: 700 1.7rem Helvetica, "Helvetica Neue", Arial, "Apple SD Gothic Neo", "Noto Sans KR", sans-serif; } .score { color: var(--navy); font: 700 3.2rem Helvetica, "Helvetica Neue", Arial, "Apple SD Gothic Neo", "Noto Sans KR", sans-serif; } .narrative p + p { margin-top: 12px; } h3 { color: var(--blue); font: 700 1.1rem Helvetica, "Helvetica Neue", Arial, "Apple SD Gothic Neo", "Noto Sans KR", sans-serif; margin: 0 0 10px; } @media print { body { background: white; } .toolbar { display: none; } main { width: 100%; margin: 0; } .report { border: 0; box-shadow: none; padding: 0; } }</style></head><body><nav class="toolbar"><a href="/assessment/policy?source=${encodeURIComponent(result.policySource)}">Dashboard</a><button type="button" onclick="window.print()">Print / Save PDF</button></nav><main><article class="report"><header><div><div class="eyebrow">AGAF AI Governance Assessment</div><h1>${escapeHtml(result.organizationName)} AI 정책 평가보고서</h1></div><div class="meta"><div><span>Policy Source</span><strong>${escapeHtml(result.policySource)}</strong></div><div><span>평가일</span><strong>${escapeHtml(assessmentDate())}</strong></div><div><span>보고서명</span><strong>${escapeHtml(result.organizationName)}_AI정책_평가보고서.pdf</strong></div><div><span>평가 기준</span><strong>${escapeHtml(result.policySourceAssessmentBasis.assessmentStandards.join(" · "))}</strong></div></div></header><section><h2>Section 1 · 정책 개요</h2><p>${escapeHtml(result.policyOverview)}</p></section><section><h2>Section 2 · 정책 분석 결과</h2>${renderSummaryText(result.executiveSummary)}</section><section><h2>Section 3 · 규제 대응 수준 / AI Governance Score</h2><div class="score-grid"><div class="metric"><span>AI Governance Score</span><strong class="score">${result.totalScore}/${result.maxScore}</strong></div><div class="metric"><span>Regulatory Readiness</span><strong>${escapeHtml(result.regulatoryReadiness)}</strong></div><div class="metric"><span>Audit Readiness</span><strong>${escapeHtml(result.auditReadiness)}</strong></div></div></section><section><h2>Section 4 · 5 Domain Analysis</h2><div class="score-grid">${result.domainScores.map((item) => `<div class="metric"><span>${escapeHtml(item.label)}</span><strong>${item.score}/${item.maxScore}</strong></div>`).join("")}</div></section><section><h2>Section 5 · Gap Analysis</h2><ul>${result.gapAnalysis.map((gap) => `<li>${escapeHtml(gap)}</li>`).join("")}</ul></section><section><h2>Section 6 · Highest Priority Risks</h2><ul>${result.priorityRisks.map((risk) => `<li><strong>${escapeHtml(risk.label)}</strong> (${risk.score}%) - ${escapeHtml(risk.description)}</li>`).join("")}</ul></section><section><h2>Section 7 · Recommended Actions</h2><ul>${result.recommendedActions.flatMap((group) => group.actions).map((action) => `<li>${escapeHtml(action)}</li>`).join("")}</ul></section><section><h2>Section 8 · Roadmap</h2><div class="roadmap">${result.recommendedActions.map((group) => `<div class="roadmap-card"><h3>${escapeHtml(group.label)}</h3><ul>${group.actions.map((action) => `<li>${escapeHtml(action)}</li>`).join("")}</ul></div>`).join("")}</div></section></article></main></body></html>`;
}
