...
const domainRecommendations: Record<DomainScore["domain"], Array<Omit<Recommendation, "priority">>> = {
  ...
  AI_RISK_MANAGEMENT: [
    { title: "AI Approval Workflow", reason: "비인가 AI 행동은 직접적인 운영 리스크와 시스템 변경 리스크로 연결됩니다.", expectedOutcome: "AI 행동에 대한 승인 통제 확보" },
    { title: "Runtime Monitoring", reason: "AI 행동 중 모니터링이 없으면 의도하지 않은 작업을 조기에 탐지하기 어렵습니다.", expectedOutcome: "AI 행동의 실시간 가시성 확보" },
    { title: "AIGuard PoC", reason: "고위험 AI 행동은 정책 기반 통제 가능성을 작은 범위에서 검증해야 합니다.", expectedOutcome: "AI Governance 적용 가능성 검증" },
  ],
};

const industryPriorityDomains: Record<IndustryType, DomainScore["domain"][]> = {
  TECHNOLOGY: ["STRATEGIC_GOVERNANCE", "AI_RISK_MANAGEMENT"],
  FINANCIAL: ["AI_RISK_MANAGEMENT", "FINANCIAL_ACTIONS"],
  HEALTHCARE: ["PRIVACY_DATA_PROTECTION", "MODEL_GOVERNANCE_HUMAN_OVERSIGHT"],
  PUBLIC: ["MODEL_GOVERNANCE_HUMAN_OVERSIGHT"],
  MANUFACTURING: ["FINANCIAL_ACTIONS"],
};

const industryReasons: Partial<Record<IndustryType, Partial<Record<DomainScore["domain"], string>>>> = {
  FINANCIAL: { AI_RISK_MANAGEMENT: "금융산업에서는 비인가 AI 행동이 직접적인 운영 리스크로 연결됩니다." },
  HEALTHCARE: { PRIVACY_DATA_PROTECTION: "의료 데이터는 높은 개인정보 민감도를 가집니다." },
  PUBLIC: { MODEL_GOVERNANCE_HUMAN_OVERSIGHT: "공공부문은 투명성과 감사 가능성이 신뢰 확보의 핵심입니다." },
  MANUFACTURING: { FINANCIAL_ACTIONS: "제조 환경에서는 권한 없는 변경이 생산 운영과 품질 리스크로 이어질 수 있습니다." },
  TECHNOLOGY: { STRATEGIC_GOVERNANCE: "기술 조직은 AI 도입 속도가 빨라 사용 정책 부재 시 Shadow AI가 빠르게 확산됩니다.", AI_RISK_MANAGEMENT: "기술 조직의 AI는 개발·운영 시스템에 접근할 가능성이 높아 실행 통제가 중요합니다." },
};
...
export function quickWinsFor(result: AssessmentResult & { industry?: IndustryType }): string[] {
  ...
  const quickWinByDomain: Record<DomainScore["domain"], string> = {
    STRATEGIC_GOVERNANCE: "AI 사용 정책 문서 작성",
    PRIVACY_DATA_PROTECTION: "민감정보 입력 가이드 배포",
    FINANCIAL_ACTIONS: "승인 절차 정의",
    MODEL_GOVERNANCE_HUMAN_OVERSIGHT: "감사 로그 보관기간 정의",
    AI_RISK_MANAGEMENT: "AI 행동 승인 기준 초안 작성",
  };
...
export function roadmapFor(result: AssessmentResult & { industry?: IndustryType }): RoadmapItem[] {
  ...
  return [
    ...
    { day: "90 Days", title: recommendations[2]?.title ?? industryProfiles[result.industry ?? "TECHNOLOGY"].phases.at(-1)?.title ?? "AI Governance" },
  ];
}
